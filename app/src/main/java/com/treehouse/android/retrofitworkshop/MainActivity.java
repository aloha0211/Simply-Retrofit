...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void fetchAccountImages() {
        ...
        Service.getAthedApi().images(OAuthUtil.get(OAuthUtil.ACCOUNT_USERNAME), 0)
    ...
    private void upload() {
        ...
        try {
            BufferedSource source = Okio.buffer(Okio.source(getAssets().open("sample_image")));
            byte[] image = source.readByteArray();
            Service.getAthedApi().uploadImages(
    ...
    private void uploadAnon() {
        ...
        try {
            BufferedSource source = Okio.buffer(Okio.source(getAssets().open("sample_image")));
            byte[] image = source.readByteArray();
            Service.getAnonApi().uploadImages(
                    RequestBody.create(MediaType.parse("image/jpeg"), image)
            ).enqueue(new Callback<Basic<Image>>() {
                @Override
                public void onResponse(Call<Basic<Image>> call, Response<Basic<Image>> response) {
